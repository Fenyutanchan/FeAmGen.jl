...
#########################################
function generate_SP_combo( num_str::String, ind_mom_list::Array{Basic})::Array{Basic}
#########################################
  ...
  r1 = count( "q1", num_str )
  r2 = count( "q2", num_str )
  num_mom = length(ind_mom_list)

  # term_q1_k = SP(q1,k1)+SP(q1,k2)+...
  term_q1_k = 0
  for i in 1:num_mom
    term_q1_k = SP(q1,ind_mom_list[i])+term_q1_k
  end # for

  # term_q2_k = SP(q2,k1)+SP(q2,k2)+...
  term_q2_k = 0
  for i in 1:num_mom
    term_q2_k = SP(q2,ind_mom_list[i])+term_q2_k
  end # for
  ...
  for q1_q2_exp in 0:min(r1,r2)
    ...
    for q1_q1_exp in 0:max_q1_q1_exp
      ...
      for q2_q2_exp in 0:max_q2_q2_exp
        ...
        total_term = total_term+term_q1_k^q1_k_exp*term_q2_k^q2_k_exp*SP(q1,q1)^q1_q1_exp*SP(q2,q2)^q2_q2_exp*SP(q1,q2)^q1_q2_exp
      end#for q2_q2_exp
    end#for q1_q1_exp
  end#for q1_q2_exp
...
end # function generate_SP_combo
